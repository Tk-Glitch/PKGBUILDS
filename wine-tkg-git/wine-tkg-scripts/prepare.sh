...
_prepare() {
	...
	if [ "$_lol920_fix" == "true" ]; then
	  if git merge-base --is-ancestor 39138478fdd93cc0dfc1e83b85784bc468e8d237 HEAD; then
...
EOM
	...
	if [ "$_EXTERNAL_INSTALL" == "true" ] && [ "$_EXTERNAL_INSTALL_TYPE" == "proton" ] && [ "$_unfrog" != "true" ]; then
	  ...
	  if [ "$_steamvr_support" == "true" ]; then
	    if git merge-base --is-ancestor c736321633c6a247b406be50b1780ca0439ef8b0 HEAD; then
